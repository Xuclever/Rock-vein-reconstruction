...
path = r"F:/cyx/Deeplearning/MASK_RCNN_2.5.0-master/output/5/stitch-images547-Binary/stitched_image2.png"
...
# Create a colour image for drawing the skeleton
color_image = np.zeros((image.shape[0], image.shape[1], 3), dtype=np.uint8)

# Draw the skeleton in white
skeleton_coords = np.column_stack(np.where(skeleton))
for coord in skeleton_coords:
    color_image[coord[0], coord[1]] = [255, 255, 255]

# Display the original image and the skeleton image
...
io.imsave('skeleton_image.png', color_image)